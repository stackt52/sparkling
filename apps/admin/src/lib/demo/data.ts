...
export const DEMO_PROFILES: DemoProfile[] = [
  ...
  person('seed_sipho_staff', 'technician', 'Sipho Ndlovu', 'sipho.n@sparklingauto.co.za', '+27820000014', [OUTLET_MEN], ['wash', 'paint', 'panel'], 'available'),
...
export const QUOTATIONS: Quotation[] = [
  ...
  { id: '20000000-0000-4000-8000-000000000006', ref: 'QT-2026-0040', customer_id: 'seed_naledi', customer_name: 'Naledi Mokoena', vehicle: { id: 'd0000000-0000-4000-8000-000000000003', registration_no: 'HR 88 TS GP', make: 'Suzuki', model: 'Swift' }, outlet: MEN_REF, category: 'Scratch', description: 'Trolley scratch on the rear quarter panel, down to the primer.', status: 'converted', amount_cents: 145000, line_items: [{ label: 'Scratch repair & blend', description: 'Rear quarter panel: sand, prime and respray in Pearl Arctic White.', category: 'scratch', service_id: SVC.SPOT_REPAIR, amount_cents: 145000 }], items_note: 'Drop the car off any weekday morning — allow 2 working days.', assessor_id: 'seed_sipho_staff', assessor_name: 'Sipho Ndlovu', valid_until: daysAgo(-9).slice(0, 10), quoted_at: daysAgo(2), decided_at: daysAgo(1), decision_note: 'Booked in for next week', decision_source: 'app', decision_by_name: 'Naledi Mokoena', attachments: [], created_at: daysAgo(3), work_order_ref: 'WO-2026-4826' },
  { id: '20000000-0000-4000-8000-000000000004', ref: 'QT-2026-0039', customer_id: 'seed_daniel', customer_name: 'Daniel Botha', vehicle: { id: 'd0000000-0000-4000-8000-000000000006', registration_no: 'CJ 118-334', make: 'Ford', model: 'Ranger' }, outlet: MEN_REF, category: 'Panel', description: 'Tailgate dented and scratched from towing accident.', status: 'accepted', amount_cents: 485000, line_items: [{ label: 'Tailgate panel repair', service_id: SVC.PDR, amount_cents: 385000 }, { label: 'Respray & blend', service_id: SVC.SPOT_REPAIR, amount_cents: 100000 }], assessor_id: 'seed_sipho_staff', assessor_name: 'Sipho Ndlovu', valid_until: daysAgo(-10).slice(0, 10), quoted_at: daysAgo(3), decided_at: rel(-130), decision_note: null, decision_source: 'app', decision_by_name: 'Daniel Botha', attachments: [], created_at: daysAgo(4), work_order_ref: 'WO-2026-4827' },
...
/* ---------- work orders ---------- */
/**
 * `checkedIn`: minutes relative to now for `checked_in_at` (migration 0015); `null` = still awaiting the explicit
 * "car checked in" confirmation. Every confirmed booking and every accepted quotation has its work order at once
 * (awaiting check-in); the booking / work-order check-in stamps it.
 */
const wo = (id: string, ref: string, outlet_id: string, booking_ref: string | null, quotation_ref: string | null, customer: string, vehicle_id: string, service_id: string, status: WorkStatus, priority: 1 | 2 | 3, bay: string | null, tpl: string, assignee: string | null, eta: number | null, due: number | null, started: number | null, blocked: string | null, stepsDone: number, checkedIn: number | null = -60): WorkOrder => {
  ...
  return {
    ...
    checked_in_at: checkedIn === null ? null : rel(checkedIn), checked_in_by_name: checkedIn === null ? null : outlet_id === OUTLET_GLV ? 'Thandi Khumalo' : 'Johan Botha',
  };
};
...
export const WORK_ORDERS: WorkOrder[] = [
  ...
  // Converted from QT-2026-0040 — the car is not on site yet, so the board shows "Awaiting check-in" and Assign is refused.
  wo('30000000-0000-4000-8000-000000000009', 'WO-2026-4826', OUTLET_MEN, null, 'QT-2026-0040', 'seed_naledi', 'd0000000-0000-4000-8000-000000000003', SVC.SPOT_REPAIR, 'queued', 2, null, TPL_BODY, null, null, 3 * 24 * 60, null, null, 0, null),
  // QT-2026-0039 was accepted in the app 2 h ago: its work order exists at once (awaiting check-in) and the quote is still `accepted` and unpaid.
  wo('30000000-0000-4000-8000-000000000010', 'WO-2026-4827', OUTLET_MEN, null, 'QT-2026-0039', 'seed_daniel', 'd0000000-0000-4000-8000-000000000006', SVC.PDR, 'queued', 2, null, TPL_BODY, null, null, 3 * 24 * 60, null, null, 0, null),
  // SPK-2026-0097 (cash on collection, 14:00) is confirmed, so its work order is already on the board awaiting check-in.
  wo('30000000-0000-4000-8000-000000000011', 'WO-2026-4828', OUTLET_MEN, 'SPK-2026-0097', null, 'seed_ayanda', 'd0000000-0000-4000-8000-000000000007', SVC.SPARKLING_WASH, 'queued', 2, null, TPL_VALET, null, 240 + 45, 240 + 60, null, null, 0, null),
];
